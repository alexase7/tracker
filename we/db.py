...
class DB:
    ...
    def init_db(self):
        ...
        if "vat_rate" not in cols:
            cur.execute("ALTER TABLE products ADD COLUMN vat_rate REAL DEFAULT 19;")
    ...
    def list_products(self):
        cur = self.conn.cursor()
        cur.execute("SELECT id, name, sale_price, COALESCE(vat_rate, 19) FROM products;")
        return cur.fetchall()
    ...
    def upsert_product(self, name: str):
        ...
        cur.execute(
            """
            INSERT INTO products(name, vat_rate)
            VALUES (?, 19)
    ...
    def set_vat_rate(self, pid: int, vat_rate: float):
        self.conn.execute("UPDATE products SET vat_rate=? WHERE id=?;", (vat_rate, pid))
        self.conn.commit()

    def get_vat_rate(self, pid: int):
        cur = self.conn.cursor()
        cur.execute("SELECT COALESCE(vat_rate, 19) FROM products WHERE id=?;", (pid,))
        r = cur.fetchone()
        return float(r[0]) if r and r[0] is not None else 19.0
